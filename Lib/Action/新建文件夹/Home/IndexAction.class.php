...
class IndexAction extends BaseAction
{
	...
	function hcttjjjss(){
			...
			$this->display('./GZphp/Tpl/Home/Default/Hcetuij_index.html');
  ...
  //dump($alist);exit;
		...
		//获取服务保障文章
		 $fwbzlist=M('news')->where("class1=482")->order('no_order desc,id desc')->limit(9)->select();
			...
			//获取保卖车
			$bmmccc=M('news')->field('id,title,imgurl')->where("class1=461 and gz_deffcdzbmm like '%保卖车%'")->order('no_order desc,id desc')->limit(9)->select();
			$this->assign('bmmccc',$bmmccc);
			//dump($necliissfq);exit;
			
			//获取超值车
			$bmmccczz=M('news')->field('id,title,imgurl')->where("class1=461 and gz_deffcdzbmm like '%超值%'")->order('no_order desc,id desc')->limit(9)->select();
