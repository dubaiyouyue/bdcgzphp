...
class SssercAction extends BaseAction
{
    public function index()
    {
		...
		global $foldername;
		global $namemark_tpl;
		$blist=$this->bannerlists(485);
		$getinfolamu=$this->get_column_info(485);$this->assign('getinfolamu',$getinfolamu);
		//dump($getinfolamu);exit;
		   $lanmu=M('column')->where('bigclass=485')->order('no_order asc,id asc')->select();
		
		  /*  $news=M('news')->where("class1=485")->select();
