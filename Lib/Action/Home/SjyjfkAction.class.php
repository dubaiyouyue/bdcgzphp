...
class SjyjfkAction extends BaseAction
{
    public function index()
    {
		...
		global $foldername;
		global $namemark_tpl;
		$blist=$this->bannerlists(487);
		$sjblist=$this->bannerlists(487,2);
		$getinfolamu=$this->get_column_info(487);$this->assign('getinfolamu',$getinfolamu);
		//dump($getinfolamu);exit;
		   $lanmu=M('column')->where('bigclass=487')->order('no_order asc,id asc')->select();
		
		  /*  $news=M('news')->where("class1=487")->select();
