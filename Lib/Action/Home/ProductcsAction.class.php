...
class ProductcsAction extends BaseAction
{
	...
	function getzmpp(){
		...
		if($zm){
			...
			foreach($g as $k=>$v){
				echo '<span class="buy_br_c_each"><a href="/index.php/Home/Productcs/index/cid/'.$cid.'/oneid/'.$v['logo'].'.html">'.$v['name'].'</a></span>';
			}
		}else{
			//热门
    ...
    public function index(){
		...
		$gz_deffcddyycxgz_deffcddyy=$this->unescapessss(cookie('deffcddyy'));
		
		if(!$gz_deffcddyy && $gz_deffcddyycxgz_deffcddyy) $gz_deffcddyy=$gz_deffcddyycxgz_deffcddyy;
		...
		$sjblist=$this->bannerlists(461,2);
		...
		if($cid==461) $cclslss='class1';
			...
			//排序
			$pxorder=$this->unescapessss(cookie('pxorder'));
				switch ($pxorder){
					case 'id':
					  $order='id desc'; //最新发布
					  break;  
					case 'jiageold':
					  $order='jiageold+0 asc'; //价格升序
					  break;
					case 'sppsjjtime':
					  $order='sppsjjtime desc'; //车龄
					  break;
					case 'licuser':
					  $order='licuser+0 asc'; //里程
					  break;
					default:
					$order='';
				}
			...
			if($minjiage && $maxjiage){
				$w['jiageold']=array('between',array($minjiage,$maxjiage));
			}else if($minjiage) $w['jiageold'] = 'jiageold+0>'.$minjiage;//array('egt',$minjiage);
			else if($maxjiage) $w['jiageold'] = 'jiageold+0<'.$maxjiage;//array('elt',$maxjiage);
		...
		if($ajax=='ajax') $this->display('./GZphp/Tpl/Home/'.SHOUJIZHANOK.'/Productcs_ajax.html');
