...
class SjmfgjAction extends BaseAction
{
    public function index()
    {
		...
		global $foldername;
		global $namemark_tpl;
		$blist=$this->bannerlists(488);
		$sjblist=$this->bannerlists(488,2);
		$getinfolamu=$this->get_column_info(488);$this->assign('getinfolamu',$getinfolamu);
		//dump($getinfolamu);exit;
		   $lanmu=M('column')->where('bigclass=488')->order('no_order asc,id asc')->select();
		
		  /*  $news=M('news')->where("class1=488")->select();
