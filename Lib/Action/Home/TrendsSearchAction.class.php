...
class TrendsSearchAction extends BaseAction
{
    public function index()
    {
		...
		$sjblist=$this->bannerlists(140,2);
		$getinfolamu=$this->get_column_info(140);$this->assign('getinfolamu',$getinfolamu);
		  $lanmu=M('column')->where('bigclass=140')->order('no_order asc,id asc')->select();
		     foreach($lanmu as $kuccc=>&$kuccv){
				...
				if($kuccv['id']){
					$kuccv['listcon']=M('news')->where('class2='.$kuccv['id'])->limit(4)->order('no_order asc,id desc')->select();
       ...
       $p=$_GET['p']+0;
		...
		$allpage=$this->Pagenewinfo($cclslss,$cid,0,$k,$kwww);
      ...
      //	print_r($list);exit();
		...
		if(!$foldername) $foldername='TrendsSearch';
		if(!$namemark_tpl) $namemark_tpl='index';
